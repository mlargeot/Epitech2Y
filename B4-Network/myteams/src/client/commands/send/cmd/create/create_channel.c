...
static void check_response(response_t *response)
{
    ...
    if (response->status_code == CREATED) {
        ...
        client_event_channel_created(uuid, name,
            xml_get_node_by_tag(xml->root, "description")->inner_text);
        client_print_channel_created(uuid, name,
            xml_get_node_by_tag(xml->root, "description")->inner_text);
        xml_free(xml);
    } else {
...
void create_channel(client_t *client, char **command)
{
    ...
    if (xml == NULL)
        return;
    xml_new_node(xml->root, "team_uuid", strdup(client->scope.teams));
    xml_new_node(xml->root, "name", strdup(command[1]));
    xml_new_node(xml->root, "description", strdup(command[2]));
    response = fetch_url(client, "/channels", POST, xml_str(xml));
